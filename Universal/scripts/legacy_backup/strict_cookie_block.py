...
js_content = """document.addEventListener("DOMContentLoaded", function() {
    ...
    // Wait for CSS to load before injecting HTML to prevent FOUC (flash of unstyled content)
    link.onload = () => {
        const widgetHTML = `
        ...
        const loadPreferences = () => {
            ...
            if (prefs) {
                ...
                // Hard block on index page after 15 seconds
                setTimeout(() => {
                    isForced = true;
...
print(f"Updated {file_path} for strict index blocking")
